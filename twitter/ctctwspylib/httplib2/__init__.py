...
*MODIFIED (Line 446): httplib2 has been slightly modified to ensure that encoded credentials
are returned in one line.  The current version of httplib2 (httplib2-python3-0.5.0 - JUL 28 09)
does not work with the Python Client Library.
"""
...
class BasicAuthentication(Authentication):
    ...
    def request(self, method, request_uri, headers, content):
        ...
        headers['authorization'] = 'Basic ' + base64.b64encode(("%s:%s" % self.credentials).encode('utf-8')).strip().decode('utf-8').replace('\n','')
