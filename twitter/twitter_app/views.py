...
from twitter.control.models import Oauth
...
def return_(request):
    ...
    oauth_token = oauth.OAuthToken.from_string(access_token.to_string())
    
    key = oauth_token.key
    secret = oauth_token.secret
#    response = HttpResponseRedirect(reverse('twitter_oauth_friend_list'))
    oauth_obj = Oauth(user=request.user, token=oauth_token.key, 
                      token_secret=oauth_token.secret)
    oauth_obj.save()
#    request.session['access_token'] = access_token.to_string()
    return HttpResponseRedirect(request.session.get('return_url'))
